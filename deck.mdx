...
<h1>How to do Good, Without doing Anything<span style={{visibility: 'hidden'}}>*</span></h1>

<p style={{visibility: 'hidden'}}>(sort of)</p>
...
- and in a way, you'll be able to say you're doing a lot of good
...
<h1>How to do Good, Without doing Anything<span>*</span></h1>
...
You need to have a good [foundation](https://www.youtube.com/watch?v=2iijtPDtV0s) for accessibilty and testing.
...
```js 1 file=./code/e2e-test-0.0.1.js
```

```js 2 file=./code/e2e-test-0.0.1.js
```

```js 3 file=./code/e2e-test-0.0.1.js
```
...
```js 7:9 file=./code/e2e-test-0.0.1.js
```
...
- Using cypress let's write a simple e2e test for our same slack sign up form
- we want to know that our slack sign up component works using only a keyboard... NEXT
- query for our accessible input ...NEXT...and type in an email address 
- NEXT...tab to our submit submit button NEXT and hit enter NEXT
- Finally we check that our accessible alert is updated with the success message.
...
<!-- hypothetical test walkthroughs -->

<CodeSurfer theme={github}>

```js 8:17 file=./code/e2e-test-0.0.2.js
```

```js 4:6 file=./code/e2e-test-0.0.2.js
```

```js 19:21 file=./code/e2e-test-0.0.2.js
```
...
- Simple and small, but tests the entire user flow of our slack sign up form using just a keyboard.
- that's what we want to check in our tests: how is a user interacting with my app, how can i translate that into tests and validate that it works? do it through an accessibility lense
- NEXT: could also test that our user is alerted of errors like in our integration test
...
- For e2e tests, think about your users, think about the flow of your app, and how you want to validate.
...
- we covered the three types of tests.
- Unit tests for component apis and behaviors.
- Integration tests for components that interact or depend on other components
- and e2e tests for validating entire using flows.
...
- NEXT: In testing we covered unit, integration, e2e.  When to use each, what to test for each category.
...
- It can feel really overwhelming if this is the first time you've thought about the accessibility of your products, and are taking stock of the work that needs to happen to bring it up to standards and create reliable processes around them.
...
- eventually, you'll be able to say you're doing a lot of good
- without doing anything
